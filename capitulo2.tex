\chapter{Máquinas de fabricación CNC}

Las máquinas automatizadas de fabricación digital o CNCs son \textbf{sistemas mecatrónicos} programables que ejecutan procesos de fabricación mediante un set de instrucciones definido en un programa. Incluyen tecnologías como la impresión 3D, torneado, fresado y corte láser, entre otros. Se caracterizan por su precisión, repetibilidad y autonomía operativa, lo que les permite \textbf{fabricar piezas diversas y complejas de manera sencilla}. En este capítulo se tratarán las características generales de una máquina de este tipo, así como los conceptos técnicos clave para describir sus procesos.

\section{Flujo del proceso de fabricación digital}

Las tecnologías de fabricación digital se fundamentan en un \textbf{proceso computarizado} que abarca desde el diseño hasta la producción de la pieza final (Figura \ref{digfab}). Este proceso comienza con la modelación de la componente deseada mediante un software de \textbf{diseño asistido por computadora} (CAD, Computer-Aided Design). Este tipo de herramientas no solo permite crear y visualizar el modelo tridimensional de la pieza, sino también analizar su integración dentro de un sistema ensamblado y evaluar su desempeño mecánico mediante técnicas como el \textbf{análisis por elementos finitos} (FEA, Finite Element Analysis).
...
Las principales tecnologías CNC se dividen en dos tipos: \textbf{aditivas y sustractivas}. Las primeras crean un volumen agregando material, mientras que las segundas lo obtienen mediante la remoción de material a partir de un formato comercial. Cabe destacar que existen máquinas \textbf{híbridas} que combinan ambos enfoques, y además, otras que realizan procesos de conformado (como estampado, plegado o forja), así como operaciones de ensamble.
...
La operación de de fabricación estará determinada por la \textbf{herramienta} equipada en la máquina. Por ejemplo, si se monta un extrusor de plástico la máquina podría funcionar como impresora 3D, y si se cambia por un \textbf{spindle} (portaherramienta rotatorio) la misma máquina podría actuar como fresadora. Si bien, esta máquina podría llevar a cabo ambos procesos, la naturaleza de sus subsistemas la hará más idónea para uno de estos.
...
El eje de un motor eléctrico de tipo \textbf{stepper} (Figura \ref{stpi}) rota un \textbf{arco constante} (o un paso, típicamente 1.8$^\circ$) cuando reciben una señal de control. De esta manera, es posible establecer un sistema de control de \textbf{lazo abierto} (sin la parte inferior del loop en la Figura \ref{controlup}) para imponer el perfil cinemático del rotor y la posición final, ofreciendo una opción más sencilla y económica de control. 
...
La corriente a través de estas bobinas determinará el torque que puede ejercer el motor contra la carga. En caso de que la carga supere el torque del motor, este no rotará cuando corresponda o se moverá en la dirección opuesta, perdiendo pasos sin que el controlador lo note directamente, induciendo un error en el proceso. Por lo anterior, versiones más costosas de estos motores incluyen un sistema de sensado, en caso de que la precisión sea crítica o no se trabaje necesariamente en condiciones completamente determinadas.
...
Los procesos de fabricación digital ejecutan una serie de operaciones destinadas a conformar las piezas requeridas. Cada operación se implementa mediante trayectorias de herramienta o \textbf{toolpaths}, las que a su vez se definen por los parámetros configurables en el CAM o definidos en el firmware de la máquina. El grado de control sobre estos parámetros depende del nivel funcional del CAM: algunos sistemas exponen decenas o cientos de variables, mientras que otros ofrecen sólo los parámetros esenciales.
...
\begin{itemize} 
	...
	\item \textbf{Parámetros de pieza (estructurales)}: específicos de la salida geométrica y funcional, especialmente en fabricación aditiva: altura de capa, ancho de pasada, porcentaje y patrón de relleno, espesor de pared y estrategias de soporte. \end{itemize}
...
El toolpath es la ruta que seguirá la herramienta para ejecutar las distintas operaciones definidas en el CAM. Estas rutas dependen del principio de funcionamiento del proceso y son específicas de cada tecnología; no obstante, comparten características comunes que permiten su análisis y parametrización. 
...
Esta estrategia de relleno o de corte debe optimizar no solo la eficiencia cinemática del trazado, sino también la integridad y la calidad del proceso. En operaciones que generan calor, la distribución de las pasadas condiciona las \textbf{zonas de concentración térmica} y, por ende, la degradación o alteración microestructural del material. En procesos que producen anisotropía (por ejemplo, fabricación aditiva o mecanizado por capas), la geometría y orientación del patrón de relleno influyen directamente en la respuesta mecánica de la pieza, su rigidez y su resistencia a la fatiga, además de otros atributos funcionales y superficiales. Se observan distintas estrategias de escaneo en la Figura \ref{scansfig}.
...
\section{Capacidades y calidad}

En general, en los procesos de fabricación, un aumento en los requisitos de \textbf{calidad} de un componente tiende a provocar un incremento \textit{no lineal} en el tiempo de procesamiento (Figura \ref{qualfig}).

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\linewidth]{imgs/qual.png}
	\caption{La inversión en tiempo crece de manera no lineal con la calidad en un proceso de fabricación.}
	\label{qualfig}
\end{figure}

Para argumentar tal afirmación, empecemos definiendo calidad. La calidad es un concepto amplio que abarca múltiples dimensiones. Para los fines de este texto, se entenderá principalmente como el grado en que un proceso de fabricación logra producir componentes que cumplen con los siguientes criterios:

\begin{enumerate}
	\item Las dimensiones generales del componente se ajustan a las especificaciones requeridas.
	\item El componente cumple con las normas o estándares técnicos aplicables.
	\item El acabado superficial se encuentra dentro de los límites establecidos por los requisitos de diseño.
\end{enumerate}

El primero punto corresponde a la \textbf{tolerancia}. Se define entonces como la \textbf{variación dimensional permitida} respecto a las especificaciones nominales de una pieza (Figura \ref{tolerfig}), dentro de la cual el componente aún se considera funcional y conforme al diseño. Representa el rango aceptable de desviaciones en dimensiones lineales, angulares, formas o posiciones, y se establece en función de los requerimientos de ensamblaje, desempeño mecánico y costos de producción. Por ejemplo, una tolerancia común en impresoras 3D de escritorio tipo FDM es de $\pm 0.5 \,\mathrm{mm}$, lo cual debe tomarse en cuenta al momento del diseño, especialmente para asegurar encajes.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\linewidth]{imgs/toler.png}
	\caption{Tolerancia para diámetros en un eje. Se acepta una variación mayor o menor a 0.01 unidades en la sección izquierda, mientras que en la derecha es de 0.02 unidades.}
	\label{tolerfig}
\end{figure}


Por ejemplo, un engranaje fabricado mediante mecanizado CNC presenta un diámetro de 50.05 mm, una dureza de 60 HRC y una rugosidad superficial de 0.8 $\mu$m Ra. Estas características se encuentran dentro de los rangos especificados por el diseño (50.00$\pm$0.10 mm, dureza mínima de 60 HRC, y rugosidad máxima de 1.0$\mu$m Ra), por lo que el componente cumple con los criterios de conformidad establecidos en cuanto a dimensiones, propiedades mecánicas y calidad superficial.

Definir márgenes más estrechos para las características de los componentes fabricados, es decir, exigir mayor calidad, conlleva dos consecuencias principales: por un lado, se incrementa la probabilidad de obtener componentes no conformes (y, por tanto, descartados); por otro, se requiere un \textbf{control más riguroso del proceso}. En consecuencia, será necesario un mayor esfuerzo para mantener los \textit{set-points} de los parámetros operativos dentro de límites precisos. Esta exigencia se vuelve aún más desafiante a medida que aumenta la velocidad o complejidad de los fenómenos físicos involucrados en el proceso, lo cual dificulta la estabilización de las condiciones de operación.

Por ejemplo, durante el mecanizado de una pieza con una herramienta de corte, se genera calor de forma inherente debido a la fricción y la deformación plástica del material. A bajas velocidades de corte, el aumento de temperatura suele ser moderado y la disipación térmica es suficiente para evitar efectos significativos. Sin embargo, al incrementar la velocidad del proceso, la energía por unidad de tiempo (potencia de corte) también aumenta, mientras que el tiempo disponible para disipar el calor disminuye. Como resultado, se produce una acumulación térmica en la zona de corte, lo que puede provocar dilataciones térmicas no deseadas en la pieza e incluso modificaciones en la microestructura del material, afectando la calidad dimensional y mecánica del componente.

Ciertamente, la velocidad del proceso es uno de los atributos clave al seleccionar sistemas para conformar una cadena productiva, ya que se busca maximizar la producción. La velocidad máxima de una máquina está determinada por el hardware, específicamente por la capacidad de los motores que integran la cadena cinemática y los sistemas de transmisión. Sin embargo, esta velocidad está limitada por las exigencias de calidad del proceso, ya que aceleraciones demasiado altas pueden generar vibraciones que comprometan la precisión y consistencia del producto final.
...
El juego mecánico (backlash, en inglés) se refiere al movimiento relativo no deseado entre dos componentes mecánicos cuando se invierte la dirección del movimiento en un sistema de transmisión. Este fenómeno ocurre, por ejemplo, en engranajes, husillos, tuercas o acoplamientos, y se manifiesta como un pequeño desplazamiento libre antes de que el movimiento se transmita efectivamente. No es viable eliminar completamente el juego mecánico, es consecuencia de tolerancias de fabricación, desgaste o diseño intencional, y puede afectar negativamente la precisión y repetibilidad en sistemas de control numérico y máquinas de fabricación digital. Una característica positiva del juego mecánico es facilitar o hacer posible el ensamble de un sistema.
...
En una arquitectura cinemática basada en motores paso a paso, el error de exactitud está condicionado por la naturaleza discreta del paso angular. Cada pulso desplaza el eje un ángulo fijo, introduciendo un sesgo sistemático de hasta $\pm \tfrac{1}{2}$ paso o micropaso frente al valor objetivo. De este modo, ciertas posiciones teóricas resultan inalcanzables para el sistema; no obstante, la importancia de este sesgo depende de la tolerancia admitida en el proceso.
...
Finalmente, la elección entre una tecnología u otra depende de la estructura de costos de la empresa y del equilibrio que esta desee lograr entre el nivel de calidad requerido (y su valor agregado) y el volumen de producción alcanzable. Esta decisión implica considerar no solo los aspectos técnicos del proceso, sino también su viabilidad económica, de modo que se maximice la eficiencia sin comprometer los estándares del producto ni la competitividad en el mercado.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{imgs/prototip.png}
