...
Es importante destacar que tanto DLP como MSLA, al proyectar cada sección completa de la pieza, alcanzan velocidades de impresión superiores a las de un láser focalizado, pues \textbf{el tiempo por capa no depende ni del tamaño ni de la cantidad de objetos}. De este modo, a diferencia de FDM, imprimir múltiples piezas en un solo lote reduce directamente el tiempo de fabricación por pieza. Cabe destacar que los subsistemas mecánicos de este tipo de máquinas son bastante sencillos, ya que el mecanismo tiene pocas partes móviles.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\linewidth]{imgs/jolou.png}
...
Otro fenómeno evaluar durante la orientación de la pieza es el efecto ventosa: cavidades cerradas con la concavidad orientada hacia abajo (hacia el tanque) generan presión negativa y fricción al separarse la plataforma, aumentando las fuerzas de adhesión y elevando el riesgo de desprendimiento o deformación del componente. Por tanto, es necesario observar en la simulación e identificar zonas potencialmente selladas, para elegir una orientación que minimice este efecto. Alternativamente, se puede agregar en el diseño orificios de ventilación en estas cavidades.

\subsection{Procesamiento en CAM}

El procesamiento del modelo en el software CAM suele requerir una configuración de parámetros más limitada que en FDM, debido a la simplicidad operacional de la máquina y a la necesidad de mantener condiciones seguras y reproducibles del proceso. Normalmente el CAM permite definir la altura de capa, parametrizar estrategias de soportes y ajustar un conjunto reducido de parámetros críticos (tiempos de exposición, desplazamientos Z y secuencias de separación), delegando en el firmware de la impresora el control fino de la ejecución.

Al igual que ocurre con los filamentos en FDM, cada fabricante de resina proporciona recomendaciones para los parámetros (tiempos de exposición, velocidades de separación, parámetros de soporte y tolerancias de postcurado) que hacen viable la fabricación en condiciones óptimas. En sistemas cerrados basta seleccionar la resina en la interfaz para aplicar automáticamente la configuración recomendada. 

Es muy importante destacar que esta tecnología de fabricación \textbf{no usa habitualmente G-Code}, pues las instrucciones son en parte mapas de bits que contienen las secciones transversales a proyectar.

\subsection{Flujo del proceso}

A diferencia del proceso FDM, que es relativamente directo y produce piezas casi finalizadas tras la impresión, el proceso SLA implica \textbf{múltiples etapas de postprocesamiento} para alcanzar que la pieza alcance su forma y propiedades mecánicas finales.
...
El siguiente paso es el \textbf{postcurado UV}, donde la pieza se expone a radiación ultravioleta bajo condiciones controladas de temperatura y tiempo, habitualmente en un horno especializado. Este tratamiento permite aumentar el grado de entrecruzamiento de la red polimérica, lo que se traduce en una mayor rigidez, dureza superficial y estabilidad térmica. Sin embargo, debido a la baja conductividad térmica del polímero y la limitada penetración de la radiación UV, se genera un \textbf{gradiente de curado}, con una mayor conversión en las capas externas que en el núcleo de la pieza. Este efecto debe considerarse en el diseño estructural y en el análisis del comportamiento mecánico, especialmente en piezas de gran espesor.
...
Las formulaciones de resinas para SLA suelen otorgar una mayor diversidad de propiedades mecánicas que las disponibles en FDM.
