...
De la experiencia, sabemos que al aplicar una fuerza sobre un cuerpo este podría deformarse hasta romperse, sin embargo sabemos que existe una clara diferencia entre, por ejemplo, quebrar la rama de un árbol que el tronco entero, a pesar de estar constituidos del mismo material. Luego, es claro que la geometría juega un papel importante en la resistencia y debe ser normalizada para el análisis del \textbf{estrés mecánico}.

A este estrés lo llamaremos \textbf{esfuerzo}, y por lo general se denota con la letra griega sigma $\sigma$. Los tipos de esfuerzos los podemos clasificar por la manera en que actúan sobre un cuerpo. Por ejemplo, si un esfuerzo actúa únicamente en dirección perpendicular a una superficie del cuerpo lo llamamos axial, y se describe como:
...
Con $F$ la fuerza aplicada sobre el cuerpo y $A$ la sección normal en la cual actúa esta fuerza. La medida de esfuerzo entonces da cuenta de la distribución de la fuerza sobre un área, y es una medida clave para el análisis mecánico de un cuerpo, asociada a la unidad Pascales $\mathrm{Pa}$ del sistema internacional.

Dadas las condiciones de un problema, el esfuerzo se comparará con diferentes valores: propios del material, impuestos como parámetros de diseño, o de acuerdo a normas. A partir de tales métricas se podrá cuantificar la performance del componente respecto a la solicitación mecánica.

De la experiencia de romper una rama, también sabemos que importa la forma en que actúa la fuerza, pues claramente es más sencillo romper la rama flectándola desde sus extremos que tirándola. Así, el cálculo del esfuerzo dependerá de 3 factores clave: \textbf{Geometría}, \textbf{tipo de carga} y \textbf{condiciones de apoyo}. En la Figura \ref{fig:1} se ven tipos de carga sobre un cuerpo, en la \ref{fig:2}, de apoyo. La ecuación \ref{eq1} corresponde a una condición de carga axial. 
...
Entenderemos como deformación al cambio geométrico de un cuerpo, no relacionado con la pérdida de materia, por ejemplo, por una reacción química, corrosión, o ciertos cambios microestructurales, entre otros. Siendo más específico, son de particular interés para la ingeniería mecánica las deformaciones ocasionadas por alguna carga física o térmica.

Una forma de categorizar los tipos de deformación por carga es respecto a la permanencia del cambio, donde si es reversible (al dejar de aplicar la carga), se le llama \textbf{deformación elástica}, y en caso contrario, \textbf{deformación plástica}. La parte elástica es resultado de interacciones atómicas y/o moleculares (atracción, repulsión, desenrrollamiento de cadenas) con la fuerza aplicada, mientras que lo plástico está relacionado con el desplazamiento permanente de átomos o moléculas en la estructura. Por lo anterior, siempre existe deformación elástica antes de que se dé deformación plástica a medida que incrementa el esfuerzo. La deformación se denotará con la letra griega epsilon $\varepsilon$. 

La relación entre esfuerzo y deformación ($\sigma - \varepsilon$) en la materia se estudia particularmente en el marco general que establece la disciplina de Mecánica de Medios Contínuos, donde se formulan \textbf{ecuaciones constitutivas} para representar esta relación. Frecuentemente, sobre este marco realizan simplificaciones con el fin de facilitar el análisis, como la linealidad en deformaciones elásticas en sólidos. 

Probablemente ya has estudiado una de estas representaciones sencillas asociada a la deformación en resortes, la Ley de Hooke:
...
Para el material exhibido en la Figura \ref{fig:2}, se observa una región inicial lineal, la cual corresponderá a la parte elástica. Al límite de esta región se le conoce como \textbf{esfuerzo de fluencia} o límite elástico (\textit{yield strength}), simbolizado como $\sigma_{y}$, y es una \textbf{propiedad mecánica} de los materiales. La pendiente es entonces el módulo de Young $E$.

De la misma manera, al esfuerzo máximo en la curva le llamaremos \textbf{resistencia última a la tracción} $\sigma_{UTS}$ (\textit{ultimate strength}). Más allá de este punto, se da un modo de deformación local conocido como \textbf{estrangulamiento} o \textit{necking}, donde el área transversal se reduce, hasta la fractura del espécimen. Si bien, se observa que el esfuerzo baja en la curva, esto es así porque para el cálculo se considera el área inicial $A_{0}$ en lugar de la real. Con tal corrección, se observaría que el esfuerzo sigue aumentando.
...
Esto aumenta la solicitud al material. La magnitud de este factor dependerá de normas y de la certeza en los cálculos empleados, intentando mantener el valor lo más reducido posible, pues su consecuencia podría implicar un \textbf{sobredimensionamiento} costoso del componente o sistema.
...
Este crecimiento de grieta a través de la fatiga está relacionado con \textbf{cargas cíclicas} y debilita al material, reduciendo su vida útil a pesar de que los esfuerzos no sean superiores a $\sigma_{y}$. El origen, tasa de crecimiento y daño inducido por estas cargas depende de múltiples variables, por ejemplo:
...
La fatiga como parámetro de diseño permite calcular la \textbf{vida útil} del componente, mediante, por ejemplo, el uso de la curva de vida amplitud-ciclos o diagrama de Wöhler (Figura \ref{fig:4}).
...
En el ejemplo se observan 2 aleaciones metálicas ampliamente estudiadas, acero y aluminio. Nótese el \textit{endurance limit} (límite de fatiga, frecuentemente simbolizado como $\sigma_{ew}$), el cual indica que para amplitudes de esfuerzo inferiores a este el material tiene \textbf{vida infinita}.

Dado que esto depende en parte del tamaño de grieta, el material también podría fallar de manera estática si es que posee una grieta inicial. Esta grieta avanzaría de forma rápida e inestable hasta la ruptura, sin necesidad de ciclos de carga en función de la \textbf{resistencia a la propagación de grietas} del material.
...
Entenderemos como concentrador de esfuerzo en ingeniería mecánica a cualquier discontinuidad geométrica o cambio abrupto de sección, como orificios, estrechamientos, cambios de diámetro, chaveteros, roscas, entre otros, que provocan una distribución no uniforme de esfuerzos en su vecindad, generando peaks de esfuerzo local muy superiores al esfuerzo nominal calculado mediante fórmulas elementales de resistencia de materiales. 

Esta amplificación se cuantifica mediante el \textbf{factor de concentración de esfuerzos} $K_t$, definido como la razón entre el esfuerzo máximo local y el esfuerzo nominal ($K_t = \sigma_{max}/\sigma_{nom}$), y su magnitud depende principalmente de la geometría de la discontinuidad y no del material. Los concentradores de esfuerzo son particularmente críticos en el diseño frente a fatiga, ya que las grietas por fatiga nuclean preferentemente en estas zonas de alto gradiente de esfuerzo, incluso cuando el esfuerzo nominal permanece muy por debajo del límite de fluencia del material.

Otra causa para la falla de materiales es la \textbf{termofluencia} o \textbf{creep}, que corresponde a una deformación progresiva y dependiente del tiempo que sufre un material bajo carga constante a temperaturas que superan cierto umbral respecto a su temperatura de fusión, lo que puede culminar en falla sin necesidad de aumentar la tensión aplicada. 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{imgs/indent.png}
    \caption{Esquema para el ensayo de dureza Brinell con indentador esférico.}
    \label{dur}
\end{figure}
...
Dado que estos fenómenos se presentan principalmente en la superficie de un material, se han desarrollado técnicas o \textbf{tratamientos de endurecimiento superficial} que generan una capa externa de elevada dureza y resistencia al desgaste, mientras el núcleo conserva ductilidad y tenacidad, optimizando así la durabilidad y seguridad de componentes.

La dureza es una medida especialmente útil para caracterizar elastómeros porque cuantifica su resistencia a la deformación por indentación y, por ende, su facilidad para deformarse bajo carga. Esta propiedad se reporta habitualmente en subescalas de la escala Shore, cada una diseñada para un rango de rigidez diferente: Shore OO para materiales muy blandos como geles y espumas suaves; Shore A para elastómeros blandos a semiduros, gomas y muchos cueros; y Shore D para materiales duros, termoplásticos rígidos y gomas estructurales (Figura \ref{smutfig}). 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.85\linewidth]{imgs/smut.png}
	\caption{Escalas de dureza Shore. \href{https://www.smooth-on.com/page/durometer-shore-hardness-scale/}{Fuente}}
	\label{smutfig}
\end{figure}
...
\begin{itemize}
  ...
  \item Calor específico, $c_p\,\mathrm{[J/(kg\,K)]}$, energía necesaria para elevar 1 K la temperatura de 1 kg del material.
...
La \textbf{difusividad térmica}, \(\alpha\), sintetiza estas tres magnitudes y mide la rapidez con que se propaga una perturbación térmica:
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.65\linewidth]{imgs/tempgraf.png}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.85\linewidth]{imgs/bonds.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\linewidth]{imgs/crist.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\linewidth]{imgs/micr.png}
...
En la formación de ingeniería se presta especial atención a los materiales de uso industrial más extendido. El acero ocupa un lugar central, \textbf{aleación} (mezcla de elementos en la que al menos uno es metálico) de hierro y carbono. Las variaciones en su composición (contenido de carbono y elementos de aleación) y en los \textbf{procesos de fabricación} (laminado, forja, tratamientos térmicos) definen la microestructura resultante y, con ello, sus propiedades mecánicas. Otras aleaciones estudiadas son las de aluminio, bronce y latón.
...
El fenómeno principal que rige el comportamiento mecánico de los polímeros a bajas y medias temperaturas es la \textbf{transición vítrea} (\(T_{g}\)) (Figura \ref{vitr}). Se trata de un cambio abrupto en la rigidez y en las propiedades viscoelásticas cuando el material pasa el umbral establecido por dicha temperatura.
...
No es posible alcanzar cristalinidad total en polímeros porque sus largas cadenas presentan irregularidades (extremos libres, ramificaciones y defectos) que impiden el empaquetamiento ordenado a lo largo de toda la macromolécula. Además, los entrelazamientos moleculares generan zonas amorfas donde las cadenas no pueden reordenarse en redes cristalinas. Como resultado, siempre coexisten regiones rígidas y ordenadas con áreas desordenadas. En general, los termoestables serán amorfos, pues los enlaces cruzados limitan el ordenamiento regular de las cadenas.

En los polímeros semicristalinos, el nivel de cristalinidad (habitualmente un 30–70$\%$) influye en la densidad, la tenacidad y la capacidad de absorber agua (higroscopicidad). Por el contrario, los amorfos suelen presentar una amplia ventana térmica de procesado, lo que facilita operaciones como moldeo por inyección o extrusión.
...
Para reducir el daño potencial asociado a la fotodegradación, es habitual incorporar \textbf{aditivos estabilizantes} durante la fotopolimerización. Estos compuestos actúan como protectores frente a la radiación UV, absorbiendo o disipando la energía dañina y prolongando la vida útil del polímero.