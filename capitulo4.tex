...
El colimador se caracteriza principalmente por dos parámetros: su diámetro $d_{c}$ y su distancia focal $f_{col}$. Para lograr una colimación óptima, el lente debe colocarse a una distancia $f_{col}$ respecto al origen del haz, de modo que los rayos emergentes sean paralelos y mantengan la uniformidad del perfil.
...
Para comprender la transmisión en la fibra óptica, introducimos la \textbf{Ley de Snell} (Figura \ref{fig:snell}):
...
Una vez que el rayo ha ingresado a la fibra, este puede refractarse hacia el medio externo al llegar a la pared interna. Para impedir que escape, se requiere que la luz incidente sobre esa interfaz se refracte paralelo a la fibra. Así, el \textbf{ángulo crítico} $\theta_c$ del rayo moviéndose hacia la pared se obtiene imponiendo que el ángulo transmitido sea $\pi/2$:

\begin{equation}
	n_1 \sin\theta_c = n_2 \sin\theta_{\frac{\pi}{2}}
\end{equation}
...
por lo que dentro del diámetro $d_{0g}$ se concentra aproximadamente el 86.5\% de la energía del haz gaussiano ideal.
...
Donde $MA$ es el factor de magnificación.
...
De esta manera, se define el \textbf{Diámetro de Rayleigh} (típicamente se encuentra como radio en las referencias) como:

\begin{equation}
	d_{r}=\sqrt{2}d_{0}
\end{equation}

Lo cual corresponde al diámetro en el cual el área del spot se duplica. Desarrollando los cálculos, se tiene entonces que la \textbf{profundidad de campo} (\textit{depth of field}) corresponde a:

\begin{equation}
	\Delta z_{dof} = \frac{\pi d_{0}^{2}}{2 \lambda M^{2}}=2 \frac{d_{0} f_{foc}}{d_{c}}
\end{equation}

Debe notarse que de desarrollar los cálculos sólo se llega a la mitad del valor descrito en la ecuación anterior, pero este debe multiplicarse por un factor de 2 para dar cuenta de la simetría de la distribución. 

Este parámetro $\Delta z_{dof}$ de la óptica de un sistema láser nos indica hasta que espesor de materiales podemos tratar de manera efectiva. 

\subsection{Distribución temporal}

El haz de luz se puede presentar en dos formas respecto a la naturaleza de la onda, pudiendo ser \textbf{continua} (\textit{CW, continuous wave}) o \textbf{pulsada} (\textit{PW, pulsed wave}).

La onda continua es naturalmente la más sencilla de tratar. Su potencia puede ser modulada previamente a la aplicación, y el perfil temporal es constante, luego:
...
De esta manera, si $\tau$ corresponde al tiempo de encendido de la onda, la \textbf{potencia peak} en ese lapso es:

\begin{equation}
	P_{peak} = \frac{E}{\tau} = P_{out}\delta 
\end{equation}
...
Por ello, utilizamos la \textbf{fluencia} del sistema para compararla con los umbrales característicos del material y determinar cuál será el mecanismo de interacción predominante, como se ilustra en el Cuadro \ref{fluencias}.
...
A través de este modelo, podemos hacer comparaciones como la que se observa en la Figura \ref{profundi}
...
El resultado son campos de temperatura, como se observa en la Figura \ref{rosen}. A través de las curvas de nivel, es posible estimar el ancho del corte, así como la profundidad, tras calibrar el modelo con datos experimentales.
...
Un modelo sencillo para estimar los parámetros de corte se basa en la \textbf{capacidad térmica} del material:
...
De esta manera, dado un \emph{setup} fijo, es posible determinar las velocidades admisibles para la operación deseada, como se observa en la Figura \ref{metalscort}.
...
Respecto a los métodos de corte láser, los sistemas pueden clasificarse en dos tipos según la proximidad del cabezal con el material: \textbf{de proximidad} y \textbf{remotos}. Esta distinción influye directamente en la calidad del corte, la velocidad de operación y la elección del gas de asistencia.
...
Por otro lado, los gases \textbf{reactivos}, como el oxígeno, inducen oxidación en el material, generando una reacción exotérmica que incrementa la energía disponible para el corte. En este caso, el láser solo debe suministrar periódicamente la energía de activación necesaria para mantener la reacción. Como se observa en la Figura \ref{react}, el uso de un gas reactivo puede aumentar la velocidad de corte, aunque la terminación superficial suele ser menos uniforme y el \textbf{HAZ} más amplio que en el caso de gases inertes.

La terminación de un corte por contorno generalmente presenta un estriado, así como una fracción de material fundido que puede escurrir hacia el borde inferior de la sección (Figura \ref{corte}), facilitada por los gases de evacuación. Tanto el inicio como el final del corte suelen mostrar una acumulación triangular de material donde la penetración es incompleta respecto al resto del perímetro. Esto ocurre porque, en estas zonas, la velocidad del cabezal cambia y el campo de temperaturas es relativamente más bajo. Estas irregularidades se corrigen mediante estrategias específicas, como pasadas adicionales, variación de la potencia del láser o el uso de \emph{tabs} temporales.

En los sistemas \textbf{remotos}, el láser se propaga a través de espejos y el cabezal se mantiene a mayor distancia del material, sin necesidad de gases de asistencia, aunque pueden tener esa opción. Esta configuración reduce la inercia del cabezal, lo que permite alcanzar mayores velocidades de desplazamiento y ofrece mayor seguridad, al mantener el cabezal alejado de la zona de corte.  

Los sistemas remotos requieren un enfoque óptico más preciso, ya sea mediante una distancia focal elevada o una divergencia pequeña del haz. Por lo general, se aplican en procesos de alta velocidad sobre materiales planos o en geometrías extensas, donde la precisión de posicionamiento es crítica y la interacción con gases de asistencia no es necesaria. Además, permiten cortes rápidos con menor desgaste mecánico del cabezal, aunque la densidad de energía del haz debe controlarse cuidadosamente para garantizar la penetración adecuada.
